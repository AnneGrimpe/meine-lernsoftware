...
def fach_loeschen(name):
    """
    Löscht ein Lernfach vollständig:
    - Lernbewertungen
    - Prüfungsfragen
    - Karteikarten
    - das Lernfach selbst
    ...
    try:
        ...
        supabase.table("pruefungsfragen").delete().eq(
            "fach", name
        ).execute()

        supabase.table("karten").delete().eq(
            "fach", name
        ).execute()

        # Danach das Lernfach selbst löschen
        supabase.table("faecher").delete().eq(
            "name", name
        ).execute()

        return True, (
            f'Das Lernfach "{name}" wurde vollständig gelöscht. '
            "Auch die zugehörigen Karteikarten, Prüfungsfragen "
            "und Lernfortschritte wurden entfernt."
        )

    except Exception as fehler:
        return False, (
            "Das Lernfach konnte nicht vollständig gelöscht werden: "
            f"{fehler}"
        )
...
def admin_login(bereich):
    ...
    st.warning(
        ...
        "Bitte melde dich als Administratorin an."
    )

    with st.form(f"admin_login_formular_{bereich}"):
        passwort = st.text_input(
            "Admin-Passwort:",
            type="password"
        )

        anmelden = st.form_submit_button(
            "🔐 Anmelden"
        )

        if anmelden:
            if not admin_passwort_vorhanden():
                st.error(
                    "In den Streamlit-Secrets ist noch kein "
                    "ADMIN_PASSWORD hinterlegt."
                )

            elif passwort == st.secrets["ADMIN_PASSWORD"]:
...
def admin_status_anzeigen():
    ...
    with st.sidebar:
        ...
        if st.session_state.admin_eingeloggt:
            ...
            st.info(
                "Verwaltungsbereiche sind gesperrt."
            )
...
with tab_pruefungsfragen:
    if admin_login("pruefungsfragen"):
        st.header(
...
with tab_faecher:
    if admin_login("faecher"):
        ...
        if faecher:
            ...
            st.subheader(
                "🗑️ Lernfach vollständig löschen"
            )
            ...
            st.error(
                "⚠️ Achtung: Beim vollständigen Löschen werden auch "
                "alle Karteikarten, Prüfungsfragen und gespeicherten "
                "Lernfortschritte dieses Fachs dauerhaft gelöscht."
            )

            bestaetigung = st.checkbox(
                f'Ich möchte "{fach_zum_loeschen}" wirklich vollständig löschen.',
                key="loeschen_bestaetigen"
            )

            name_bestaetigung = st.text_input(
                'Zur Sicherheit den Namen des Lernfachs eingeben:',
                placeholder=fach_zum_loeschen,
                key="loeschen_name_bestaetigung"
            )

            darf_loeschen = (
                bestaetigung
                and name_bestaetigung.strip() == fach_zum_loeschen
            )

            if st.button(
                "🗑️ Lernfach endgültig löschen",
                type="primary",
                disabled=not darf_loeschen
            ):
